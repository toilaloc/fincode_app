...
| PUT | `/v1/payments/:id/cancel` | Cancel authorization OR Refund payment |
...
#### Single Endpoint for Cancel & Refund

Fincode uses a single API endpoint for both canceling authorizations and refunding captured payments:

- **Endpoint**: `PUT /v1/payments/{id}/cancel`

| Transaction State | Action Performed | Result |
|-------------------|------------------|--------|
| `AUTHORIZED` | **Cancel** | Void the authorization (release hold) |
| `CAPTURED` | **Refund** | Refund the payment (return funds) |

**Note**: Do not look for a `/refund` endpoint, it does not exist. Use `/cancel`.
...
|------------|---------|----------|
| `E01010001` | Invalid parameter | Check request parameters |
| `E01020001` | Insufficient funds | Customer needs to use different card |
| `E01030001` | Payment not found | Verify order_id is correct |
